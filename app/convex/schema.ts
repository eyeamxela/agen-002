...
export default defineSchema({
  ...
  manifests: defineTable({ room: v.string(), docHashes: v.array(v.string()), tiers: v.string(), ttl: v.string(), state: v.string(), brief: v.string(), createdAt: v.number() }).index('by_room', ['room']),
  ...
  runs: defineTable({ agentKey: v.string(), taskId: v.optional(v.id('tasks')), state: v.string(), cost: v.optional(v.number()), saw: v.object({ instructionsV: v.number(), manifestId: v.optional(v.id('manifests')), docHashes: v.array(v.string()) }), did: v.string(), evidence: v.array(v.string()), startedAt: v.number(), endedAt: v.optional(v.number()) }).index('by_agent', ['agentKey']),
  grants: defineTable({ principal: v.string(), objectIds: v.array(v.id('brainObjects')), perms: v.array(v.string()), runId: v.optional(v.id('runs')), expiresAt: v.optional(v.number()), noDownload: v.boolean(), revokedAt: v.optional(v.number()) }).index('by_principal', ['principal']),
  ...
  cartridges: defineTable({ name: v.string(), rel: v.string(), templates: v.array(v.string()), docHashes: v.array(v.string()), exec: v.boolean(), execConsented: v.boolean(), version: v.number(), publisher: v.string(), purpose: v.string(), updatePending: v.optional(v.number()) }),
  skills: defineTable({ key: v.string(), by: v.string(), on: v.boolean(), updatedAt: v.number() }).index('by_key', ['key']),
